...
- The address-bar trust badge now reflects your current ENS verification settings when you go Back or Forward onto a name
  ...
  - A name that no longer checks out is sent to the same _RPC servers disagreed_ or _unverified_ page a fresh visit would show. That is not only about settings you changed: RPC servers that have started disagreeing block a name on their own, blocking unverified names is on out of the box, and "Continue once" covers the one visit it was clicked for — so going Back onto a name you continued past earlier puts its page up again
  ...
  - The one exception is stepping off that name's own block page, in either direction: the restored page stays put and carries the verdict on its badge instead of putting the block page straight back up
