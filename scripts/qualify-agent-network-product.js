...
function hostBaseline() {
  ...
  return {
    ...
    networkPermissionsEnabled: NETWORK_PERMISSIONS_ENABLED,
  };
}

async function selectLanAddress() {
  const candidates = process.env.FREEDOM_SANDBOX_LAN_HOST
    ? [process.env.FREEDOM_SANDBOX_LAN_HOST]
...
async function main() {
  const baseline = hostBaseline();
  emit('host', baseline);
  if (process.platform !== 'linux') throw new Error('This qualification requires Linux');
  if (!process.versions.electron) {
    throw new Error(
      'Launch through Electron in Node mode: ELECTRON_RUN_AS_NODE=1 electron <script>'
    );
  }
  if (baseline.uid === 0) throw new Error('Refusing to qualify the sandbox as root');

  const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), 'freedom-network-product-'));
  ...
  const lanAddress = await selectLanAddress();
  ...
  // ---- Product composition (only observability wrappers around real components).
  const executions = [];
  ...
  // factory, and the gate value are used unchanged; only observability wrappers are added.
  const controller = new ManagedWorkspaceController({
    ...
    runtimeOptions: { packaged: false, freedomVersion: require('../package.json').version },
    networkPermissionsEnabled: NETWORK_PERMISSIONS_ENABLED,
  });
  ...
  const service = new FreedomAgentService({
    ...
    createControllerScope: async ({ controller: automation }) => ({
      execute: automation.execute,
      getWorkspaceState: () => ({ tabIds: [], activeTabId: null }),
      getActiveTabId: () => null,
      setApprovalMode: () => {},
  ...
  const run1 = await startRun('Qualify the experimental network permission path');
  ...
  if (!NETWORK_PERMISSIONS_ENABLED) {
    ...
    const bProbeWrite = await callTool(run3, 'write', { path: 'probe.py', content: PROBE_SOURCE });
    void bProbeWrite;
    ...
    check(
      ...
      { policyCreations, leases: 2, executions: executions.length }
    );
  }
  ...
  if (!NETWORK_PERMISSIONS_ENABLED) {
    ...
    check(
      ...
      { policyCreations, executions: executions.length }
    );
  }
  ...
  // ---- Survivors and cleanup.
  const survivors = spawnSync('pgrep', ['-af', '[b]wrap|freedom-sandbox-supervisor'], {
    encoding: 'utf8',
  });
  check(
    '8-survivors',
    'no Bubblewrap or sandbox supervisor process survives the qualification',
    survivors.status === 1 && !survivors.stdout.trim(),
    { output: survivors.stdout.trim() }
  );

  await service.dispose();
  await workspacePreviewController.dispose();
  historyStore.close();
  await Promise.all([closeServer(tcpServer), closeServer(abstractServer), closeServer(unixServer)]);
  ...
  emit('executions', { records: executions });
