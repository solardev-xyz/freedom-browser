...
### Bounded native lifecycle checkpoint — completed 2026-09-08

[The final lifecycle evidence](evidence/macos-native-lifecycle-qualification-2026-09-08.md) records one passing four-case disposable-Mac campaign, using the unchanged production source and paired helper/manifest. Actual idle and running native Quit exited the browser with status zero; running Quit terminated and reaped the command before its alarm. Killing the actual browser main also terminated the command before its alarm. Supervisor loss deliberately demonstrated the remaining limitation: the command kept writing until its own 15-second expiry, while Freedom reported `WORKSPACE_SUPERVISOR_FAILED` and uncertain teardown. No watchdog command signal or browser/framework emergency cleanup produced these passes. All registered identities have known exit accounting; transient historical descendants are not completely enumerated.

The final campaign used a tightly gated test-only safeStorage-unavailable preload after native sampling identified a keychain authorization stall during setup. This capability removal does not modify the workspace/process implementation, but limits the evidence to credential-free TEST_MODE; real-keychain, live-provider and signed/notarized release behavior remain unqualified. Earlier aborted attempts, the Python launcher-to-image correction, independent Claude review and precise clock/receipt evidence remain retained.

Together with the separate 89-assertion product campaign and ordinary Jest/lint, this completes the bounded original-root ownership implementation checkpoint. It does not complete every investigation scenario listed below. Keep `best_effort / original_process_group / survivorsPossible=true / completeDescendantTermination=false`; final group-KILL EPERM remains explicit uncertainty. Escaped descendants, supervisor loss, safe startup reclamation and aggregate containment remain unresolved, and broader release policy is still pending.
