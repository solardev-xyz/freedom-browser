...
The qualification's two integration gaps are now closed in the retained corpus: managed `bash` accepts a validated workspace-relative working directory, so the canonical-directory one-shot case runs through the real Pi tool rather than calling the controller directly; and every new command receipt records the effective `none` or `full` network posture in the live result, workspace ledger, and durable Agent activity. These follow-up assertions are locally integrated and await an exact-commit rerun on the disposable Linux host. The original run results were: enabled harness 23 assertions passed, 0 failed, 1 now-resolved finding; default-off harness 11 assertions passed, 0 failed; `npm run test:agent-sandbox`: 11 suites and 92 tests passed, 3 suites and 16 macOS tests skipped; `npm run test:agent-sandbox:qualification`: descriptor closure, focused Jest, lint, and Babel transform completed inside Bubblewrap through the approved process runtime; `FREEDOM_SANDBOX_DESTRUCTIVE=1 npm run test:agent-sandbox:destructive`: 1 passed; `npm run lint`: passed; `npm test`: 226 suites and 3,962 tests passed, 8 suites and 33 tests skipped. No off-host LAN peer exists on this server, host abstract-socket exposure is inherent to `--share-net`, and aggregate CPU, memory, and PID containment remain absent; the disposable-macOS adversarial product-path gate is still outstanding.
