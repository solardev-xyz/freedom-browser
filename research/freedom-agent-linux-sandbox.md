...
The qualification's first two integration gaps are closed in the retained corpus: managed `bash` accepts a validated workspace-relative working directory, so the canonical-directory one-shot case runs through the real Pi tool rather than calling the controller directly; and every new command receipt records the effective `none` or `full` network posture in the live result, workspace ledger, and durable Agent activity. Exact candidate `bbcd53a2` was then requalified read-only on the same server. The gate-enabled product harness passed 25/25 with zero findings; the default-off run satisfied all 11 product assertions but exited one because the new durable-posture assertion incorrectly required both `full` and `none` even when the full-network schema was absent. `npm run test:agent-sandbox` passed 92/92, repository qualification passed all four workloads, the destructive gate passed 1/1, lint was clean, and the full suite passed 3,964/3,964.

That rerun exposed one real durable-state race: stopping an in-flight command produced the correct `cancelled`/`SIGKILL`/`full` SQLite ledger receipt, but the Agent turn could finalize first and leave its activity item durably `running` without the receipt. The retained corrective candidate tracks in-flight workspace outcomes, includes them in Stop's existing bounded settlement window, and reconciles any authoritative receipt before persisting the terminal turn. A unit regression reproduces the outcome-after-model-abort ordering without a Pi `tool_execution_end` event and requires the saved activity to say `Command stopped`, retain `networkPosture: full`, and precede `run_finished`; the Linux product harness independently requires that terminal cancellation receipt in durable activity. The default-off harness now requires only `none` while the gate is closed and still requires both postures while it is open.

The same run found that npm 11 installed the Electron JavaScript package without materializing Electron's lazily downloaded binary, so the harness used a locally cached archive. Freedom's root postinstall now invokes Electron 43's supplied `install-electron` binary before Electron Builder rebuilds native dependencies; a configuration regression keeps that order explicit. This requires a genuinely fresh Linux `npm ci` rerun to prove reproducibility rather than another cache-dependent workaround. No off-host LAN peer exists on this server, host abstract-socket exposure is inherent to `--share-net`, and aggregate CPU, memory, and PID containment remain absent; the disposable-macOS adversarial product-path gate is still outstanding.
