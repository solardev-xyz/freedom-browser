...
- **Packaged follow-up in progress — 2026-09-08.** [The qualification record](evidence/packaged-workspace-qualification-2026-09-08.md) distinguishes the completed development campaign from current packaged work. Correcting a symlinked dependency staging layout produced an unsigned package with verified source/native inputs and passing packaged resolver/SQLite preflight. Its first real idle lifecycle attempt failed in the credential-isolation/observer harness before native Quit; one first-observed framework child has no original exit registration. A subsequent corrected harness passed actual packaged idle native Quit with browser exit zero and all observed helper exits accounted for; the original failure and logging limitations remain retained. Packaged running, browser-loss, supervisor-loss and bounded detached evidence remains open. Linux regression is included at the user's request; the first kernel-only setup attempt stopped at Python import before namespace creation, and exact current-source staging remains approval-blocked. Only the recorded packaged idle case has passed; no full campaign or release approval is claimed.
