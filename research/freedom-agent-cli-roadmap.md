...
- **macOS adversarial release gate — outstanding.** Ordinary implementation, unit coverage, and fixed non-destructive qualification may continue on a developer Mac with the product gate closed. Before arbitrary model-directed shell execution with expanded networking is enabled for ordinary users, the exact candidate must pass on a disposable Mac: recursive-deletion canaries, direct and interpreter-mediated path escape attempts, symlink/hardlink races, detached descendants, resource-exhaustion cases, cancellation/app-exit cleanup, and real Agent-driven commands. Destructive cases must target uniquely created disposable fixtures—never `/`, a home directory, or another real host tree—and must prove protected outside-workspace canaries survived. This is a release/exposure gate, not a blocker for gated product development.
...
- `workspace_preview` now opens either a workspace-relative HTML file or a directory containing `index.html` in a visible Agent-owned tab. Freedom serves live files through a per-preview opaque `freedom-preview://` origin rather than `file://`, localhost, or an Agent-started server; calling the tool again refreshes the existing preview tab after edits. The internal token is replaced by **Workspace preview** in the address bar, history, and Agent activity.
- Preview contents remain hostile input. Every request revalidates bounded ordinary files with `lstat`, canonical containment, no-follow open, post-open inode checks, a one-link rule, a 16 MiB response ceiling, and bounded concurrency. Symlinks, hardlinks, special files, `.git`, traversal, and cross-root access fail closed. The response policy disables external connections, framing, forms, workers, objects, popups, downloads, and cross-origin navigation; wallet/Swarm/Radicle providers and privileged internal APIs are unavailable. Conversation deletion revokes the opaque origins and clears their browser storage.
- Bash output is continuously drained and bounded by the existing backend, networking remains disabled, and every command result preserves the backend, exit state, duration, truncation, termination guarantee, and deliberately honest `sideEffects` field. Freedom further bounds model-visible output before Pi sees it so Pi never spills a hidden full-output copy into host temporary storage.
...
- Focused store, controller, Pi-adapter, service, progress, history, runtime-composition, and renderer tests cover opaque persistence, exact deletion, restart interruption, relative-directory enforcement, one-time approval, cancellation, path redaction, structured activity, and unavailable-platform behavior.
...
6. **Implemented behind an experimental gate; exposure qualification remains — Grant one honest direct-network posture on both sandbox backends.** No-network remains the default and the feature is absent from Pi's schema and instructions unless `FREEDOM_EXPERIMENTAL_AGENT_NETWORK=1` is set before startup. When enabled on a supporting backend, `request_permissions` can ask for `network: full` alone or alongside executable roots for one exact command and canonical working directory. Managed `bash` now exposes that same workspace-relative directory directly, so exact-directory permits no longer require a controller-only qualification path. **Allow once** is consumed only by that matching call; **Allow for conversation** retains the bundle. The approval discloses public internet, host localhost, private/LAN connectivity, and Linux host abstract-Unix-socket reachability behind **More details**. Freedom constructs one validated full-capability base lease only while the gate is enabled, derives the fixed helper/file policy and ordinary Agent policy back to no-network, and selects the full policy only after resolving a trusted complete grant; helper reads/writes/searches therefore remain offline. Consequential consent for publication, payments, signing, messages, and account actions remains independent. Every command receipt now records the selected `none` or `full` posture through the live result, SQLite command ledger, and durable Agent activity. The execution contract recognizes `full` as indivisible while keeping `brokered` reserved. Seatbelt grants IP inbound/outbound plus narrowly named DNS/routing/TLS platform services and no general Unix-socket rule. Bubblewrap uses `--share-net`; receipts disclose that host abstract Unix sockets become reachable. The macOS corpus is green locally for public TCP, DNS, loopback, Unix-socket denial, descendant inheritance, and revocation; a remote LAN endpoint is still needed to complete the LAN claim. The 2026-09-03 non-root Bubblewrap run proved public TCP, DNS, host loopback, the server's non-loopback address, abstract-socket reachability, pathname-socket denial, descendant inheritance, and offline revocation; only a reachable off-host LAN peer remains outstanding there. On 2026-09-04 the integrated Linux product path at `bbcd53a2` (service, real Pi `bash` tool with canonical working directory, approval normalization, grant store, controller, real Bubblewrap; deterministic fake Pi session) passed the enabled gate 25/25 with zero findings and all 11 default-off product assertions. That rerun exposed two qualification-lifecycle defects rather than a boundary escape: the default-off harness unconditionally expected a `full` durable posture, and Stop could persist a workspace activity as `running` when its authoritative cancellation receipt arrived after model-turn finalization. The retained candidate gates the assertion by posture availability and makes Stop reconcile in-flight workspace receipts within its existing deadline before durable finalization. Electron 43's lazy binary materialization is now also explicit in the root postinstall before native dependency rebuilding. An exact Linux rerun of this corrective candidate remains required. Before ordinary-user exposure, the same candidate must pass the disposable-Mac adversarial release gate above with the experimental product path enabled. Partial direct-network requests continue to fail closed; finer separation requires a broker/proxy rather than command classification. The fixed non-destructive Seatbelt corpus passes 24/24 on the primary Mac, ESLint is clean, and the full unit suite matches the baseline above; the deliberately hostile macOS corpus was not run here.
...
11. **Add managed long-lived processes after exact command and network permits qualify.** A dev server needs a conversation-owned process identity, sandbox-policy lease, bounded continuously available logs, explicit start/status/stop/restart operations, shutdown and stale-start recovery, and honest platform-specific termination receipts. Freedom should allocate a stable endpoint and connect it to the preview surface. Agent may also request the separately disclosed host-loopback or LAN capability when the task intentionally involves arbitrary services outside the managed environment. Static preview remains the default whenever a server is unnecessary.
...
- **2026-09-04 — Ship direct networking only as a default-off experimental command capability.** `FREEDOM_EXPERIMENTAL_AGENT_NETWORK=1` may expose one indivisible `full` grant covering public internet, host localhost, and private/LAN addresses; Linux also discloses host abstract-Unix-socket reachability. The main process binds one-shot grants to the exact command and canonical working directory, supports explicit conversation scope, derives ordinary Agent and fixed helper policies back to no-network, and keeps consequential consent independent. Ordinary-user exposure remains blocked on the disposable-Mac adversarial product-path gate and the documented remaining LAN evidence.
