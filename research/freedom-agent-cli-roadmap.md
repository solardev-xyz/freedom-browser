...
- **Packaged macOS follow-up completed; Linux application gate open — 2026-09-08.** [The qualification record](evidence/packaged-workspace-qualification-2026-09-08.md) distinguishes the completed development campaign from current packaged work. Correcting a symlinked dependency staging layout produced an unsigned package with verified source/native inputs and passing packaged resolver/SQLite preflight. Its first real idle lifecycle attempt failed in the credential-isolation/observer harness before native Quit; one first-observed framework child has no original exit registration. A subsequent corrected harness passed actual packaged idle native Quit with browser exit zero and all observed helper exits accounted for; the original failure and logging limitations remain retained. The subsequent three-case packaged campaign also passed running Quit, browser-main loss and supervisor-loss limitation assertions with exact source/artifact checks and retained raw evidence. Running Quit and browser loss stopped the command before its alarm; supervisor loss left the command alive until self-expiry and reported failure truthfully. The single bounded detached native-Quit case then passed its limitation assertions: browser/supervisor/managed command stopped, while the escaped child continued writing until its independent alarm. Raw receipts retain native root reap, final-KILL EPERM and output-drain uncertainty; no watchdog signal ended the escaped child. All five selected packaged cases now have retained source/artifact/exit evidence, with no stronger descendant guarantee. Linux regression is included at the user's request; the first kernel-only setup attempt stopped at Python import, while a corrected single benign namespace setup/normal-exit check subsequently passed with original wrapper/init exit accounting and unchanged same-observer host state. The user subsequently approved the exact Linux source patch; transfer and fresh staging now match all six requested source tree/blob hashes, with the donor unchanged and no missing required packages in the installed manifest inventory. After a retained npm-config failure, the corrected offline directory build and actual packaged Electron SQLite preflight passed with original exit accounting and exact source/artifact checks. The first bound idle attempt then failed at the debugger capability check before entry isolation, app readiness or Quit. Browser/driver/display were killed during failed-attempt disposal, and three registered helper exits remain individually unproved. The corrected startup ordering subsequently reached and verified the actual archived entry, but its isolation installer refused an unexpected safeStorage prototype; main remained paused and no Quit occurred. Raw evidence, independent review and unchanged artifact checks are retained. The reviewed test-only correction then successfully sealed the API, passed all runtime probes and verified real packaged main/window readiness plus app-owned idle preparation. That attempt failed when a helper was first observed after the Quit request; cleanup interrupted the case, so neither a Quit pass nor a product shutdown defect is established. The next observer correction enrolled and observed both late helpers exiting, but a scan of the original browser in zombie state interrupted proof. Its subsequent original zero exit/reap is retained; five other helper exits remain unknown, so the run still failed. A narrow correction now waits for the original browser’s confirmed exit through that transition; its fresh bounded idle attempt is pending. Packaging/dependency checks are not current Freedom application lifecycle qualification. The selected unsigned, credential-free packaged macOS batch is complete; current Linux application qualification and release approval remain separate.
