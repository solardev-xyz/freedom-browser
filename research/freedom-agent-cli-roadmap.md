...
- Packaged Linux qualification lives on the unmerged `experiment/agent-workspace-sandbox-packaged-linux` branch at `41f1598c94cc9f769d73b7fb9c7e2b1819bba3c1`. The unpacked app and `.deb` package used the exact embedded Electron/Node runtime and rejected host-Node fallback. Merge only after the independent sandbox audit reviews the focused `00e343d0..41f1598c` delta.
...
The independent read-only audit of the merged foundation found no host escape and returned **proceed with blockers**. The concrete unbounded-Linux-tmpfs and blanket-macOS-sysctl blockers plus the unverified macOS Git-link/case-folding cases are addressed on the combined hardening branch. A focused re-audit must cover those fixes and the separately qualified packaged-Linux delta before product wiring merges.
...
#### Next milestone — Gated managed-workspace product slice

Build the smallest end-to-end product surface on top of the qualified contract:

1. Freedom creates a private managed workspace per conversation or project and persists its opaque identity. The tool API gives Pi only the opaque workspace ID and workspace-relative paths. This is an authority boundary, not a path-confidentiality promise: `/proc` mount metadata on Linux and ordinary host-native paths on macOS may reveal the canonical backing path to code already running inside the sandbox.
2. Add a sandboxed shell tool with a relative working directory, bounded continuously drained output, timeout, cancellation, and structured backend/termination receipts. Network remains disabled.
3. Add one Agent-native disclosure when enabling workspace execution. Ordinary writes inside the disclosed workspace do not require per-command approval; any future boundary expansion remains a separate exact approval.
4. Persist workspace and command history across browser restart, expose live command/activity state, and make stop, retry, failure, and recovery honest about the backend's termination guarantee.
5. Gate the feature to qualified macOS and Linux package/runtime combinations. Unsupported Linux packaging, Windows, or failed capability probes remain unavailable rather than falling back to an unsandboxed shell.

Acceptance scenario: Agent creates a small HTML/CSS/JavaScript site, inspects and edits it, runs deterministic validation/build commands, can be stopped and retried, and continues in the same managed workspace after Freedom restarts. Static local preview follows this slice; verified Swarm publication follows preview. User-selected existing directories are deferred until their ownership, consent, validation cost, and lifecycle model are deliberately designed.
...
- **2026-09-01 — Qualify exact packaged runtimes before enabling managed workspaces.** A packaged macOS app is sufficient for a gated integration despite remaining unsigned in the qualification build; signing/notarization adds distribution evidence rather than a stronger child sandbox. Packaged Linux `.deb` qualification is pending focused independent review before merge. AppImage support fails closed on restricted user-namespace systems until Freedom has a reviewed launcher/AppArmor solution; Electron's injected `--no-sandbox` path is never accepted.
...
- **2026-09-01 — Treat the independent sandbox audit as a hardening gate, not a repudiation of the design.** The audit reproduced Linux confinement, found no host escape, and rated the foundation **proceed with blockers**. Freedom bounds Linux tmpfs storage and makes the root read-only, narrows macOS sysctl and process visibility, verifies protected hard-link and APFS case-folding behavior, records platform-specific survivor and loopback semantics, rejects protected paths through symlinked parents, and treats all spawned-command side effects plus all later workspace contents as untrusted. The combined fixes and packaged-Linux delta require a focused re-audit before product wiring merges.
