...
- **Linux / Bubblewrap — qualified foundation.** Mount, user, PID, IPC, and network namespaces provide the intended filesystem, network, local-IPC, and namespace-scoped descendant boundary. Receipts report `linux-bubblewrap` and `terminationGuarantee: namespace_scoped`. The adversarial corpus covers direct and scripted path access, subprocess inheritance, symlink and hardlink aliases, descriptor leakage, network/DNS/localhost, timeout, cancellation, and descendant cleanup.
...
- **macOS / Seatbelt — qualified with best-effort teardown.** Both development Electron and a packaged `Freedom.app` ran through the deny-default Seatbelt backend using the exact Electron application bundle as the read-only runtime. The packaged app executed from `app.asar`, used its embedded Node through `ELECTRON_RUN_AS_NODE=1`, and could not fall back to Homebrew Node. Receipts report `macos-seatbelt` and `terminationGuarantee: best_effort`. A `setsid()` descendant may outlive process-group cancellation, but remains inside the tested filesystem and network policy; Freedom must not claim complete process-tree termination. Seatbelt/SBPL's deprecated private status remains an explicit maintenance risk.
...
Known hardening gaps remain explicit rather than hidden behind classifier prose: no aggregate CPU, memory, PID, or disk containment; no seccomp policy beyond the current backend posture; same-UID workspace TOCTOU; macOS detached-descendant survival and PGID-reuse races; host-backed temporary-storage cleanup; runtime-layout portability; and the long-term availability of Seatbelt. These do not authorize weakening the filesystem/network boundary. Resource containment and lifecycle decisions must be completed before calling the shell generally available.
...
1. Freedom creates a private managed workspace per conversation or project and persists its opaque identity. Pi receives only the opaque workspace ID and workspace-relative paths, never the host path.
