...
4. Choose the next substantial capability from observed product demand. The creation pipeline now spans Freedom-owned project storage, sandboxed build execution, standard coding tools, isolated static and managed-server preview, and direct verified Swarm publication of exact managed-workspace files. The generic policy-filtered coding environment, composable executable/network authority, managed-process substrate, and first declared-port server-preview route are implemented; Linux requalification is complete at `aa6d02a9`; the disposable-Mac adversarial release gate remains outstanding. The next implementation priorities are a tracked reusable product qualification harness, then visible controls for running commands and servers. Dependency installation, external filesystem authority, restart reattachment, and install/version/rollback workflows remain separate follow-ons. Windows containment still requires an explicit product package before site customizations, extensions, dApps, or browser malleability can be treated as end-to-end capabilities.
...
1. **Track and reuse the product qualification harness.** Preserve the existing network-product harness and consolidate the currently external managed-process, terminal-reconciliation, and server-preview scenarios into repository-owned reusable fixtures and commands. Exercise the production controller, Pi tools, history stores, and platform executor; preserve explicit platform skips and disposable-machine gates. Remote agents should rerun the same scenarios rather than reconstruct them. This is planned work, not implemented yet.
2. **Add visible process controls.** Show the conversation's running commands and servers, their current state, and a direct stop action backed by the existing process manager. Preserve automatic terminal reconciliation and honest platform-specific cancellation evidence. Restart reattachment remains separate.
3. **Qualify the same candidate on the disposable Mac when available.** Cover the expanded network permission path, managed processes, terminal reconciliation, and server previews, including the documented adversarial cases. Keep destructive qualification off the primary Mac. Record the exact tested revision before declaring this layer cross-platform qualified.
