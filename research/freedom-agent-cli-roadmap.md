...
**Implementation and partial qualification — 2026-09-10.** The isolated correction at `8e803e0097a28bf6aef2fc41222811d825fd3967` on `fix/linux-workspace-process-ownership` has passed source review but is not merged. After correcting two test-harness setup issues, the disposable-host facilities case passed with original lifetime exit/retirement/reap evidence. The next case failed before command release: Bubblewrap could not execute the gate helper (`ENOENT`). The campaign stopped; natural command exits, missing-command handling, Stop and control EOF remain unqualified. A bounded diagnostic established the gate bytes and interpreter paths exist; correlated historical AppArmor audit records identify deleted-entry exec denials. A named gate-copy correction is being prepared, retaining pinned descriptor authority and a read-only root; no host policy or alias change is proposed. [The partial qualification record](evidence/linux-workspace-owner-qualification-2026-09-10.md) preserves the attempts, exact identities, audit attribution and remaining scope. Broader launch/parent-loss and deployment gates below remain open.
