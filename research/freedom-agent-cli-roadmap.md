...
The embedded Pi product path is live in Freedom. The product now has durable multi-turn sessions, Agent-first and browser-first views of the same task, browser-wide Agent-tab custody, in-flight steering, a trusted resumable page-takeover interlock, fresh semantic and visual observation, semantic live-working presence, evidence-based progress/completion receipts, verified file downloads, user-authorized file uploads, Agent-native dApp wallet approval, direct Freedom wallet transfers, read-only node intelligence, explicitly disclosed raw node/application diagnostics, independently classified direct node requests, durable recovery for long-running node mutations, a first native progressively disclosed operational skill, safe specific provider failure/recovery UX, and a fail-closed **Ask when needed** website-interaction posture. All twenty-one deterministic product qualifications now pass together.
...
### Current product roadmap — 2026-09-01
...
- Agent can publish bounded text, an attached file snapshot, or a user-shared live folder through the canonical `swarm_publish` operation. Inline text retains the manual publisher's text/data semantics (`Text` in history and receipts) rather than becoming a model-invented file; attached files and folders retain their real names. The existing Swarm publish service, postage configuration, upload progress, and `freedom://publish` history remain the single implementation path; the Agent does not own a parallel uploader.
...
- The model receives only an opaque attachment resource ID. Source paths stay in the trusted main process and are absent from approvals, progress events, receipts, tool evidence, conversation history, and provider context.
...
- The progressive-disclosure `swarm-publishing` skill owns workflow knowledge: static-site index selection, public-data cautions, postage preflight and handoff, live-folder semantics, operation recovery, verification, and completion reporting. `window.swarm`, raw node calls, and filesystem access are not alternate publication paths.
...
4. Choose the next substantial capability from observed product demand. Begin the larger creation pipeline—site customizations, extensions, dApps, and browser malleability—only after scoped project storage, preview, build execution, permission review, installation/publication, versioning, and rollback have an agreed product model. Swarm publication is now a qualified deploy primitive, not yet that creation pipeline.

Current verification baseline: 209 unit suites and 3,793 tests pass, with 3 suites / 10 tests intentionally skipped. The twenty-one-scenario deterministic Agent product qualification remains green and confirms that both publication operations are present in the bounded Pi tool surface; the terminal provider-failure qualification now correctly protects the non-duplicated turn-outcome UX. Focused real-Electron checks additionally cover sandboxed PDF text/page rendering plus bounded image and PDF attachment previews, transient provider recovery with visible sanitized provider detail, an exhausted three-attempt provider failure after partial browser work, the canonical desktop/hidden screenshot path, sidebar and Agent-first composer autofocus, broad attachment versus compact pill-popover geometry, horizontally scrollable sent-attachment shelves, visible provider reconnection/lifecycle presence, and overflow-safe long node receipts. Manual real-provider smoke passes for visual observation, file/folder/PDF attachments, the **Ask when needed** form workflow, and direct text, attached-file, and live-folder publication to Swarm.

#### Next experimental work — Scoped project workspace and sandboxed shell

The next creation-pipeline experiment should run on a temporary branch named `experiment/agent-workspace-sandbox`, created from the current `feature/freedom-automation-kernel` head. Its first implementation and adversarial qualification must happen on a disposable Linux VM with no personal data, production services, mounted host directories, browser/wallet profiles, or reusable credentials. Deliberately destructive escape attempts belong only on that disposable machine. Later macOS validation should use non-destructive synthetic canaries; there is no product reason to run destructive payloads against a developer workstation.

The product direction is real general-purpose shell access inside a hard runtime-owned boundary, not a growing catalog of approved commands. A shared Freedom execution policy should describe exact readable roots, writable roots, protected carve-outs, private temporary storage, environment inheritance, network posture, process/runtime limits, and UI/device access. The selected OS backend must apply that policy before untrusted code starts and to the complete descendant process tree. Unsupported or partially representable policies fail closed rather than silently running with weaker or no isolation.

- **Linux first:** use Bubblewrap as the primary process sandbox, with mount/user/PID/IPC/network namespaces, read-only host toolchain exposure, exact workspace writes, a private temporary directory, no network by default, and seccomp hardening. Landlock may add defense in depth or serve as a fallback only where the running ABI can represent the requested policy without semantic weakening.
- **macOS second:** use Seatbelt through `/usr/bin/sandbox-exec` as the pragmatic current backend, following the same generated policy contract and descendant inheritance model. Its deprecated interface is an explicit maintenance risk and must remain isolated behind the backend boundary. Apple's OCI-compatible `container`/Containerization stack is a future optional stronger Linux-VM mode for Apple-silicon Macs on macOS 26+, not the default host-native execution path.
- **Windows after the common contract is proven:** investigate adapting the Apache-licensed Codex elevated sandbox design—dedicated online/offline sandbox users, restricted tokens, synthetic SIDs/ACLs, firewall rules, protected credentials, and separate setup/runner helpers—with a clearly identified weaker unelevated fallback if required. Microsoft MXC remains worth monitoring but its current preview explicitly must not be treated as a security boundary.

The intent/effect classifier and Agent-native approvals remain above this boundary. They may explain or preflight a requested escalation, but command text classification never substitutes for enforced filesystem, network, process, and credential isolation. The experiment returns to the feature branch only after policy enforcement, streaming execution, cancellation/timeouts, packaging, and an adversarial corpus covering Python/Node indirection, subprocess inheritance, symlinks, environment leakage, local IPC, network escape, and resource exhaustion are credible across the supported backend scope.
...
- **2026-09-01 — Prototype general shell authority inside native OS sandboxes, beginning remotely on disposable Linux.** Freedom should not replace a coding agent's shell with a version-sensitive command catalog or trust a classifier to recognize every harmful payload. It should give arbitrary commands a common least-authority policy enforced by Bubblewrap on Linux, Seatbelt on macOS, and a credible native Windows backend, while approvals govern explicit boundary expansion. The experiment remains isolated from the product branch until fail-closed behavior and adversarial escape tests justify merging it back.
