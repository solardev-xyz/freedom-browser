...
- The expanded macOS adversarial product-path qualification remains a **distribution hold**, not a hidden runtime feature flag and not a block on ordinary development. Run it on a disposable Mac when available; never run destructive/adversarial shell, descendant, resource-exhaustion, or app-exit fixtures on the primary Mac. Windows workspace execution remains unsupported and fails closed.
...
#### Local checkpoints and restore — 2026-09-05

The user confirmed immutable exclusions for dependencies/generated output/caches/secrets, one repository per workspace, and local-only history. Checkpoints and restore are now implemented as Freedom-owned behavior, with a short built-in Agent prompt explaining their semantics rather than relying on a skill being selected.

- Freedom saves starting files before the first model changes in an enabled workspace, captures eligible edits made between turns before the next turn starts, and checkpoints eligible differences when a turn finishes. Unchanged turns create no duplicate checkpoint. Completed, interrupted, and failed turns have distinct version labels; saving never certifies a working project. The normal turn remains active until its checkpoint attempt settles. Failures retain workspace changes and expose an honest History notice instead of claiming recovery coverage.
- The Versions tab offers named snapshots, historical file inspection, and a concrete restore review listing writes/removals. A short-lived, conversation-bound token binds confirmation to the inspected current files. Restore requires an idle Agent and no live managed processes, revalidates the files, commits “Before restore” first, and then applies exact sandboxed file operations. A successful restore becomes a new commit rather than rewriting history. A failed or interrupted restore retains its backup and reports that restoration did not finish; it does not claim transactional rollback.
- Snapshot collection runs in the existing offline helper and respects Git ignores plus immutable path exclusions. Conservative content screening also excludes recognizable credential material and secret assignments. These checks cannot identify every possible secret representation. Versions are bounded to 200 files, 64 KiB per file, and 512 KiB of eligible content; exceeding the aggregate limit fails the checkpoint, while individual oversized/unsafe/secret files are excluded and left alone by restoration. Limits and additional exclusion details are visible. Ignored files are never blindly cleaned or recursively deleted.
- Main-process Git plumbing receives only screened bytes over stdin and writes protected metadata. It uses a sanitized environment, fixed commands, disabled hooks, bounded cancellable subprocesses, and rejects externally configured repositories or redirected metadata. The Agent sandbox retains read-only Git metadata. Saved records precede the atomic branch update; unreachable objects from an interrupted save are not advertised as versions. Existing externally managed branches/history are not adopted or overwritten. Version inspection is separate from the mutation lock so opening History cannot interrupt the Agent's file operations or cause a checkpoint to be skipped.
- Deterministic tests cover real Git commits and indexes, deduplication, interrupted labels, immutable/content exclusions, historical bytes, stale/replayed/foreign restore requests, preserving excluded files, backup recovery, partial failure, restart reads, and service/IPC/UI ownership and lifecycle. A benign native Seatbelt case covers actual snapshot/restore with an excluded file and a readable backup. Headless browser checks cover responsive layouts, naming versions, historical views, and review-before-restore confirmation. Full native Linux qualification remains batched with the next relevant checkpoint; no destructive qualification is run on the primary Mac.
- Follow-ons remain separate: larger or configurable snapshot limits, pagination beyond the latest 100 exposed versions, broader secret detection, richer version comparisons, process-aware restart after restoration, and external-folder/existing-repository workflows. Remote configuration/push and conventional staging/branch controls are still deferred.

#### Capability inventory — Generalize the coding environment and authority model

The numbered inventory below records completed foundations and remaining capabilities; it is not execution order. The dependency-based app workflow above has completed user acceptance; the next selected feature milestone is workspace inspection and local version history, beginning with the file tree and change inspection described below.
