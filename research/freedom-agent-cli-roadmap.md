...
**2026-09-09 implementation candidate, runtime review still open:** a repository-owned Linux x64 native lifetime owner now creates its PID namespace before Bubblewrap, gates launch behind an armed parent-loss handoff, and uses original creation pidfds with retirement-before-reap. The [native contract and disposable qualification matrix](../src/main/agent/workspace-execution/native/linux-supervisor.md) describe the mechanism, descriptor protocol, installed-facility requirements and remaining AppArmor/deployment gates. Compilation and pure/mock checks are not runtime ownership evidence. Historical results below remain scoped to their original candidates; stronger all-state qualification is not closed by this source change. New receipts condition namespace cleanup on the native original lifetime result rather than a requested signal.
