...
- The explicit `workspace_run` Pi tool executes a general shell command with a validated relative working directory through the platform-selected Bubblewrap or Seatbelt executor. Output is continuously drained and bounded by the existing backend, networking remains disabled, and every result preserves the backend, exit state, duration, truncation, termination guarantee, and deliberately honest `sideEffects` field.
