...
5. **Implemented foundation — Define one composable capability vocabulary before adding more prompts.** The trusted main-process contract now distinguishes canonical executable roots, external filesystem reads, external filesystem writes, outbound public internet, host loopback, private/LAN connectivity, and host IPC. Each kind has explicit resource/access semantics and an implementation status; only executable roots currently have an authority factory and enforcement adapter. Capabilities and prepared requests carry non-serializable provenance, expose no host path when serialized, bind invisibly to the owning conversation, and support exact-command/directory one-shot or deduplicated conversation grants. The existing executable flow now uses this generic permit store instead of a bespoke map. Reuse, cross-conversation replay, forged/serialized capability objects, invalid scopes, and any capability without a qualified policy adapter fail closed.
6. **In qualification — Prove one honest direct-network posture on both sandbox backends.** The current no-network policy remains the product fallback. The execution contract now recognizes `full` as an indivisible public-internet + host-loopback + private/LAN posture while keeping `brokered` reserved. Seatbelt grants IP inbound/outbound plus narrowly named DNS/routing/TLS platform services and no general Unix-socket rule. Bubblewrap uses `--share-net`; receipts explicitly disclose that this also makes host abstract Unix sockets reachable, so a future approval must not present it as IP-only authority. Both integration corpora exercise the grant from a descendant, public TCP, DNS, host loopback, a LAN target, Unix-socket behavior, and a subsequent offline execution to prove revocation; the existing backend corpus continues to cover cancellation and process-tree cleanup under the same executor. The macOS corpus is green locally for public TCP, DNS, loopback, Unix-socket denial, descendant inheritance, and revocation; a remote LAN endpoint is still needed to complete the LAN claim. Linux argument/profile tests are green, and the 2026-09-03 non-root Bubblewrap run on the disposable server proved public TCP, DNS, host loopback, the server's own non-loopback address, host abstract-socket reachability, pathname-socket denial, and offline revocation from a descendant Node process after correcting the staged name-service switch, which had left DNS disabled under `--share-net`; the corpus and repository qualification now obtain their Node/npm from the test process through the generic approved-executable runtime-root contract instead of assuming a system Node, and every Linux sandbox command exits zero on that server; only a reachable off-host LAN peer remains outstanding for the LAN claim. After shared permission wiring is complete, the exact macOS candidate must additionally pass the disposable-Mac adversarial release gate above before real Agent-controlled full networking is exposed. Partial direct-network requests continue to fail closed; finer separation requires a broker/proxy rather than command classification.
