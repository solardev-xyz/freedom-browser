...
- Pi sees the familiar `bash`, `read`, `write`, and `edit` coding-tool surface, but none is Pi's host implementation. Freedom supplies Pi's reviewed standard schemas and behavior with custom operations: `bash` executes through the platform-selected Bubblewrap or Seatbelt backend, while bounded text reads and exact writes/edits use a fixed Freedom helper through that same OS sandbox and opaque managed-workspace authority. Absolute/out-of-workspace paths, symlinks, multiply-linked files, special files, oversized files, and writes to protected top-level `.git` metadata fail closed. The same `read` tool continues to load exact reviewed built-in skill paths without turning skill disclosure into workspace or host-file access.
...
- First use presents one Agent-native disclosure in the composer. It explains the writable private workspace, disabled networking, and the platform's cancellation semantics. Once enabled for that conversation, ordinary commands and writes inside the same boundary do not generate per-command approval; any later authority expansion remains a separate exact decision.
...
Manual acceptance scenario: ask Agent to create a small dependency-free HTML/CSS/JavaScript site, inspect and revise it, run deterministic validation through `$FREEDOM_JAVASCRIPT_RUNTIME`, stop a long command, and continue in the same conversation after Freedom restarts. Static local preview is the next creation-pipeline slice; connecting the already-qualified Swarm publication path follows preview. User-selected existing directories remain deferred until their ownership, consent, validation cost, and lifecycle model are deliberately designed.
...
- **2026-09-02 — Present standard coding tools without granting Pi standard host authority.** Replace the model-facing `workspace_run` abstraction with Pi's expected `bash`, `read`, `write`, and `edit` contracts, backed entirely by Freedom-owned operations. Shell commands and the fixed bounded file helper both retain the qualified OS sandbox. Trusted built-in-name overrides are registered through a non-serializable process-local marker, while arbitrary custom tools remain unable to claim Pi's reserved host-tool names.
