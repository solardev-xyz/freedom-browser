...
- **No editorial framing.** An entry states what changed and where to find it; it never comments on the release, the roadmap, or the reader. Do not write theme sentences (`the first of the everyday-browsing basics this release is built around`, from a `0.8.5` draft that was reverted), intros, or connective prose between entries. A release's story is told by the order of entries and the headline's tagline, not by narration. Test: if a clause would survive with the feature name swapped for any other, it is framing; delete it. This is the most common failure of agent drafts and it goes straight to the top of the reviewer's list.
