...
- **The lead line of a multi-line block is a feature name, not a sentence.** Keep it crisp and headline-like — a few words, no trailing clause, no mechanism. Every qualifier or detail moves into the sub-bullets, which are themselves short and only as detailed as a reader scanning for a feature needs. The `0.8.5` draft had `Ad and tracker blocking, on by default, which also hides the empty spaces blocked ads leave:` and it should be `Ad and tracker blocking:` with sub-bullets `On by default` and — only if it earns its place — `Hides the empty spaces blocked ads leave`; that empty-spaces detail is arguably too fine-grained to keep at all. The only trailing text a lead line may carry is the headline entry's short dash-tagline saying what the thing is _for_ (`Native x402 payment support — pay as you browse, straight from the built-in wallet:`), already allowed by the density budget and voice rules.
...
**Scan the lead lines.** Read the section with every sub-bullet hidden: the lead lines on their own must give the reader the release's feature list. If a lead line needs a comma to make sense, it is carrying a qualifier that belongs one level down — split it into a bare feature name plus a sub-bullet (see Structure rules).
