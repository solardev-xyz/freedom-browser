...
The Linux `.pacman` package (Arch, Manjaro, EndeavourOS, Omarchy) comes out of the same `fpm` call as the `.deb` — `-t pacman` instead of `-t deb` — so both Linux legs produce one, including the arm64 leg that installs fpm from RubyGems rather than using electron-builder's x64-only bundled copy. It needs one build dependency the other Linux targets do not: fpm writes a pacman package's `.MTREE` manifest by shelling out to `bsdtar --format=mtree`, which neither GitHub's runner images nor the `node:24-trixie` container the Docker recipes use ship, so both the workflow and `dist:linux:*:docker` install `libarchive-tools` alongside fpm. Without it the build dies with a bare `Process failed: /bin/bash failed (exit code 127)`, which names nothing. Its name is electron-builder's own default pattern for the target, `${name}-${version}-${arch}.${ext}` with the `-${arch}` dropped for the default arch (`app-builder-lib/out/targets/FpmTarget.js`; nothing in `package.json` sets `build.linux.artifactName` or `build.pacman.artifactName`, so nothing overrides it): `freedom-browser-<v>.pacman` on x64 and `freedom-browser-<v>-aarch64.pacman` on arm64 — the package `name`, as for the `.deb`, not the `productName` the AppImage uses, and `builder-util`'s `getArtifactArchName` spelling of the arch for a pacman target, which is `aarch64` rather than the `arm64` the AppImage and the `.deb` use. `build.pacman.depends` **replaces** electron-builder's default dependency list rather than extending it, which is the point of setting it: the default names `http-parser`, which is not in the official Arch repositories, so `pacman -U` would refuse the package. What we set is the Arch spelling of the `deb` defaults — `gtk3`, `libnotify`, `nss`, `libxss`, `libxtst`, `xdg-utils`, `at-spi2-core`, `alsa-lib`.

`.pacman` **is** an auto-update target, unlike (say) a plain tarball: `FpmTarget`'s `supportsAutoUpdate()` lists it beside `deb` and `rpm`, so electron-builder writes `app-update.yml` and a `package-type` marker into the package's resources and adds the file to `latest-linux.yml` / `latest-linux-arm64.yml` alongside the AppImage and the `.deb`. `electron-updater`'s `autoUpdater` getter reads that marker and picks `PacmanUpdater`, which installs the downloaded package with `pacman -U --noconfirm` (once more after a `pacman -Sy` if the first attempt fails) through the same sudo/pkexec prompt the `.deb` path uses. Arch users therefore update in place like everyone else — but only if the `.pacman` files are uploaded to `freedom.baby/downloads` with everything else (§7): a manifest entry whose file is missing is a 404 on every update check.
...
- **Arch Linux / Omarchy**: `sudo pacman -U freedom-browser-<version>.pacman` (`freedom-browser-<version>-aarch64.pacman` on arm64). This one has no automated leg at all — the two Linux smoke jobs drive the `.deb` and the AppImage, and no GitHub-hosted runner image is Arch — so the `.pacman` is the one Linux artifact whose install has to be tried by hand every cycle. Run the checklist below from the installed `/opt/Freedom/freedom` as for the `.deb`.
...
Both run `electron-builder` inside a Linux container and download the matching Radicle addon for the target arch. Each invocation produces the `.AppImage`, `.deb` and `.pacman` artifacts for that arch; the `.pacman` package covers Arch Linux and Omarchy (`sudo pacman -U freedom-browser-<version>.pacman`).
