...
- **Arch Linux / Omarchy**: `sudo pacman -U freedom-browser-<version>.pacman` (`freedom-browser-<version>-arm64.pacman` on arm64). This one has no automated leg at all — the two Linux smoke jobs drive the `.deb` and the AppImage, and no GitHub-hosted runner image is Arch — so the `.pacman` is the one Linux artifact whose install has to be tried by hand every cycle. Run the checklist below from the installed `/opt/Freedom/freedom` as for the `.deb`.
