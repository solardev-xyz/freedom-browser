...
**License check.** `NOTICES`, `LICENSE_AUDIT.md`, and `licenses-audit.json` must describe what the artifacts actually contain. Most of that is now enforced: `licenses-audit.test.js` derives the bundled-component inventory from `package.json`'s `build.extraResources` and from `src/renderer/vendor/`, fails on any component that ships without being classified and attributed, cross-checks each recorded version against its pin (`fetch-ant.js`, `fetch-myotis.js`, `fetch-arti.js`, `fetch-freedom-ipfs-native.js`, `radicle-addon-version.js`, the lockfile), and rejects any GPL/AGPL-licensed vendor file or production package. It runs in the normal `npm test` matrix, so a red branch tells you before you tag.
