...
**Start from a tree that matches the lockfile, and resolve with the npm CI uses.** Two ways this step has produced a wrong answer, both of which survive a clean local run:

- **A stale `node_modules` makes `npm outdated` fiction.** Its `current` column is read from the installed tree, not the lockfile, so a checkout that has not been installed since the last release reports the *previous* release's versions and silently reclassifies majors as in-range. Run `npm ci` before `npm outdated`, and sanity-check that no `current` sits below its own declared range floor. `npm outdated`'s `latest` is also served from cache and can lag the registry — in the 0.8.6 cycle it reported `@babel/core`'s latest as 7.29.7 while the registry said 8.0.6, turning two ordinary in-range patches into apparent out-of-range majors. Confirm any surprising `latest` with `npm view <pkg> dist-tags`.
- **npm resolves optional dependencies differently across its own versions.** The 0.8.6 refresh was resolved with the npm bundled in Node 24.8 (11.6.0), which drops optional entries it will not install on the host platform. CI runs Node 24 as resolved by `actions/setup-node` — 24.21, npm 11.19.0 — whose ideal tree still contains them, so every job failed `npm ci` with `EUSAGE … Missing: @electron/windows-sign, cross-dirname, fs-extra, postject, commander`. A local `npm ci` on the older npm accepts the same lockfile, so the pre-commit run cannot catch it. Match the runner's Node before regenerating the lockfile (`nvm install 24.21.0`), and treat a lockfile diff that only *removes* optional transitives as the symptom.

**An Electron bump is never lockfile-only.** `scripts/check-electron-version.test.js` holds `package.json`'s range floor at the pinned version, so the bump needs `npm install --save-dev electron@<new>` rather than `npm update`, and `licenses-audit.test.js` records the lockfile-resolved version, so `licenses-audit.json` and `LICENSE_AUDIT.md` move in the same commit. All three fail together until they agree.
...
### Bundled binaries (Ant, freedom-ipfs, Radicle, Arti, Myotis)
...
| Myotis (`scripts/fetch-myotis.js`)                    | `scripts/myotis-release.json` — `releaseTag`, the `abi` gate, the `SHA256SUMS` digest and a per-target digest for all five addons    |
...
Myotis is the one binary whose bump can require **code** changes, because the loader gates on an exact engine ABI. If `abi` moves in `scripts/myotis-release.json`, `src/main/myotis/myotis-child.js`'s `EXPECTED_ABI` and `checkpoint-store.js`'s `nativeCheckpointApi` move with it — and the latter means generations recorded against the old number are preserved and replaced rather than resumed, so the upgrade re-syncs once and that belongs in the changelog. Read the release notes across the **whole** range for what each ABI step changed: v0.1.11's 26 -> 29 made `eth_call` enforce its block argument (previously ignored by the Node addon) and refuse a NULL `to`. The 0.8.6 bump also found the ABI hard-coded in two places the pin does not reach — `scripts/verify-myotis-checkpoint-addon.js`, which fails `npm run myotis:download` outright, and `ci.yml`'s `myotis-addon-load` step, which fails every platform in CI and cannot fail locally. Both now read `abi` from the pin; keep it that way rather than adding a third literal. Myotis is also Apache-2.0 with an upstream `NOTICE`, so §4(d) applies on every bump (see the note above).

For each binary/addon that's behind, re-run its fetch script (`npm run ant:download` / `ipfs:download` / `radicle:download` / `tor:download` / `myotis:download`, the Arti one being the build above) and verify the result still passes `npm run check-binaries`. Downloaded binary directories are gitignored, so the refresh usually produces no file-tree change. Document versions in the changelog and the matching build commit.
...
- Do **not** leave an empty `## [Unreleased]` section behind, and do not expect the next feature PR to bring it back: under the fragment flow no pull request edits `CHANGELOG.md` at all, so nothing re-introduces the heading until the next release assembles. `changelog-process.md` step 9 is where it comes back — add it above the latest released version before assembling, or `npm run changelog:assemble` exits 1 (dry run and `--write` alike) with `CHANGELOG.md has no '## [Unreleased]' heading to assemble into`.
...
- _Releaser working in their own checkout with an agent:_ leave the `CHANGELOG.md` edits — the assembled fragments plus the drafted section, `changelog-process.md` step 9 — unstaged, present the diff, create the `docs(changelog): …` commit only after explicit approval. Corrections before approval are edits in the working tree, not amends.
- _Agent working on its own branch (the alan flow used for 0.8.5):_ the agent opens a PR **against `release/<version>`** (explicit `--base`; the default is `main`) touching only `CHANGELOG.md` and the `changelog.d/` fragments it consumes — the assemble and the `git rm changelog.d/*--*.md` of `changelog-process.md` step 9 belong in that same PR, so the merge approves the assembled text and retires the fragments together — with a per-entry "verified against" list in the description. The PR is the presentation; the releaser's merge is the approval. Corrections land as further commits on the PR branch — do not amend — and the PR description must state which entries changed and any judgement calls left for the releaser (category placement, whether to announce a feature whose PR is still open, link targets). Expect more than one round: the 0.8.5 changelog took an alan review plus two maintainer-directed fix rounds before merge. `alan fix <repo> <pr> "<numbered instructions>"` is the tool for those rounds.
...
**UI consistency audit**, once per cycle, on the release branch before the first release candidate is tagged. The 0.8.5 audit found 28 issues (#223–#242, #249–#260), and CI now catches the theme half of that class: `theme-parity.spec.js` renders every surface the tour reaches in both themes and fails on a page that ignores the Appearance setting or text below WCAG AA contrast, `renderer-screenshots.spec.js` diffs committed screenshot baselines in both themes, and `renderer-styles.test.js` rejects colour literals outside the token files (#261). What CI cannot see is sibling drift — surfaces that are each legible but disagree with each other: three sidebar header layouts, three counter conventions, mixed button verbs, heading sizes, focus rings. Nor can it see a baseline that was committed looking wrong, since a screenshot test only reports change from it, or a surface the tour does not visit. That is what this audit is for. Run the tour in both themes from the release branch:
