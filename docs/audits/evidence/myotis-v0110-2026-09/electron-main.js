  ...
  while (Date.now() < deadline) {
    ...
    if (ready && Date.now() >= nextRead) {
      ...
      try {
        ...
        if (result?.beaconChainVerified === true && result.blsVerified === true &&
            result.peerProofValid === true && !result.failReason) return true;
...
async function main() {
  ...
  for (const [kind, contents] of [
    ...
    const passed = !started && status.state === 'recovery-blocked' && status.recovery?.reason === 'ownership' &&
      !ready && unchanged && stopped && files.length === 1 && files[0] === '.freedom-myotis-owner';
  ...
  await fs.writeFile(path.join(directory, 'anchor.json'), JSON.stringify({ schemaVersion: 1, nativeCheckpointApi: 26, chainId, generation, origin: 'verified', checkpoint }), { flag: 'wx' });
  ...
  const marker = JSON.parse(await fs.readFile(path.join(baseDir, 'verified-sync', fresh.generation,
    chainId === 1 ? 'sync-anchor.json' : 'sync-anchor-gnosis.json'), 'utf8'));
  if (marker.checkpointRoot !== fresh.checkpoint.root || marker.checkpointSlot !== fresh.checkpoint.slot)
    throw new Error('Native marker does not match authenticated checkpoint');
  emit('generation-after-recovery', { record: fresh, marker, changed: fresh.generation !== generation });
...
main().catch((error) => emit('harness-error', { message: error.message, code: error.code || null })).finally(async () => {
  ...
  success = success && Array.isArray(stopped) && stopped.every((value) => value === true);
