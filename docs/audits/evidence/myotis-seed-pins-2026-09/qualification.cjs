...
async function waitForEns(phase, budgetMs = 180000) {
  const start = Date.now();
  let signature;
  let firstReady;
  let nextRead = 0;
  const { resolveEnsAddress, clearEnsResolutionCaches } = require(
    path.join(repo, 'src/main/ens-resolver')
  );
...
async function main() {
  runDir = await fs.mkdtemp(path.join(os.tmpdir(), 'freedom-myotis-seeds-'));
  ...
  try {
    assert(await manager.startMyotis({ dataDir: baseDir }));
    await waitForEns('warm-stale-recovery', 180000);
    assert(inherited);
    const current = await generation(baseDir);
    ...
    const seedLogs = log
      .split('\n')
      .filter((line) => /seed pins \(\d+\) (applied|refused)/.test(line));
    emit('seed-pin-receipts', {
      lines: seedLogs.map((line) => line.slice(line.indexOf('[myotis]'))),
    });
    assert(seedLogs.length >= (process.argv[3] ? 2 : 4));
    assert(seedLogs.every((line) => line.includes('seed pins (5) applied')));
    success = true;
  } finally {
    watcher.close();
  }
