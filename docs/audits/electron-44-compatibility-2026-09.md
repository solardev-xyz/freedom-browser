...
> **Status, as of 2026-09-08.** This report was written on the trial branch of
> [#289](https://github.com/solardev-xyz/freedom-browser/pull/289) against
> Electron **44.2.0**, and is kept here as the B1/B2 baseline record. Both
> in-repo blockers were cleared in
> [#290](https://github.com/solardev-xyz/freedom-browser/pull/290) (`node-abi`
> `4.32.0` → `4.35.0`, commit `9fa85465`; the `clipboard` port plus its
> both-surfaces tests, commits `e8ee81f1`/`acd47611`), and the bump itself
> landed as **44.3.0** in
> [#298](https://github.com/solardev-xyz/freedom-browser/pull/298). The §5
> **no-go** verdict is therefore historical — it describes the repo before #290
> — and the 44.2.0 → 44.3.0 delta (two `Breaking API Changes (44.0)` rows added
> upstream after 44.2.0 shipped, plus the 44.3.0 behaviour changes that carry no
> breaking-changes row) lives in #298's description, not here. Everything else
> below is as observed on 2026-09-08 and dated in place; upstream state drifts.
