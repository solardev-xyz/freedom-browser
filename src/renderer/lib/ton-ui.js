...
const updateTonInfoVisibility = () => {
  if (!tonInfoPanel) return;
  const status = state.currentTonStatus;
  const visible =
    status === 'running' ||
    status === 'starting' ||
    (status === 'error' && Boolean(getDisplayMessage('ton')));
  tonInfoPanel.classList.toggle('visible', visible);
};
...
export const updateTonUi = (status, payload = {}) => {
  ...
  tonToggleSwitch.classList.remove('running', 'starting');

  switch (status) {
    case 'running':
    case 'starting':
      tonToggleSwitch.classList.add('running');
      tonToggleSwitch.classList.toggle('starting', status === 'starting');
      if (payload.proxyPort && tonProxyPort) {
        tonProxyPort.textContent = String(payload.proxyPort);
      }
      if (payload.version && tonVersionText) {
        tonVersionText.textContent = versionText(payload.version);
      }
      break;
    case 'error':
    ...
    default:
      ...
      if (tonProxyPort) tonProxyPort.textContent = UNKNOWN;
      if (tonVersionText) tonVersionText.textContent = UNKNOWN;
      break;
  }
  updateTonInfoVisibility();
};
...
export const initTonUi = () => {
  ...
  if (window.ton) {
    window.ton
      ...
      .then(({ available }) => {
        ...
        if (!available) pushDebug('TON binary not found - toggle disabled');
      })
      .catch((err) => {
        tonBinaryAvailable = false;
        setToggleDisabled(true);
        pushDebug(`TON binary check failed: ${err.message}`);
      });
  } else {
    tonBinaryAvailable = false;
    setToggleDisabled(true);
  ...
  if (!tonUiInitialized) {
    ...
    tonToggleBtn?.addEventListener('click', () => {
      ...
      if (state.currentTonStatus === 'running' || state.currentTonStatus === 'starting') {
