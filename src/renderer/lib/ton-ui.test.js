...
describe('ton-ui', () => {
  ...
  test('updateTonUi stopped clears port and version', async () => {
    ...
    expect(ctx.elements.tonProxyPort.textContent).toBe('Unknown');
    expect(ctx.elements.tonVersionText.textContent).toBe('Unknown');
  ...
  test('marks startup separately from a fully running proxy', async () => {
    const ctx = await loadTonModule();
    ctx.mod.initTonUi();

    ctx.mod.updateTonUi('starting');

    expect(ctx.elements.tonToggleSwitch.classList.contains('running')).toBe(true);
    expect(ctx.elements.tonToggleSwitch.classList.contains('starting')).toBe(true);
  });
  ...
  test('registers the status subscription without redundant polling', async () => {
    const ctx = await loadTonModule({
      currentTonStatus: 'stopped',
    });
    ...
    expect(ctx.tonApi.getStatus).not.toHaveBeenCalled();
  });

  test('keeps an error message visible in the info panel', async () => {
    const ctx = await loadTonModule({ statusMessage: 'Proxy failed to start' });
    ctx.mod.initTonUi();

    ctx.mod.updateTonUi('error', { error: 'Startup timed out' });
