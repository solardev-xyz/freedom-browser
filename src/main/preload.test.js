...
describe('preload', () => {
  ...
  test('exposes the preload bridges and routes direct wrappers to ipcRenderer', async () => {
    ...
    const invokeCases = [
      ...
      [exposures.electronAPI, 'steerAgent', ['run_test', 'Focus on sources'], IPC.AGENT_STEER, [{ runId: 'run_test', prompt: 'Focus on sources' }]],
      ...
      [exposures.electronAPI, 'getAgentAttachmentPreview', ['conversation_123', 'attachment_123'], IPC.AGENT_ATTACHMENTS_PREVIEW, [{ conversationId: 'conversation_123', resourceId: 'attachment_123' }]],
  ...
  test('registers electronAPI, github bridge, and service registry listeners with cleanup', () => {
    ...
    const listenerCases = [
      ...
      [exposures.electronAPI, 'onAgentEvent', IPC.AGENT_EVENT, [{ type: 'assistant_text_delta', text: 'Hi' }], [{ type: 'assistant_text_delta', text: 'Hi' }]],
      [exposures.electronAPI, 'onAgentProviderAuthEvent', IPC.AGENT_PROVIDER_AUTH_EVENT, [{ type: 'device_code', userCode: 'ABCD-1234' }], [{ type: 'device_code', userCode: 'ABCD-1234' }]],
